...
export function activate(context: vscode.ExtensionContext): void {
    ...
    if (debug) {
        console.log('恭喜，您的扩展 "插入序列(中文版)" 现已激活！');
    }
...
function InsertSequenceCommand({
    ...
    function getValidInputRegExp(): { [ key: string ]: any } {
        ...
        const ruleTemplate: RuleTemplate = {
            ...
            dateformat:
                '(?:G{1,5}|y{1,5}|R{1,5}|u{1,5}|Q{1,5}|q{1,2}|M{1,5}|L{1,5}|w{1,2}|l{1,2}|d{1,2}|E{1,6}|i{1,5}|e{1,6}|c{1,6}|a{1,5}|b{1,5}|B{1,5}|h{1,2}|H{1,2}|k{1,2}|K{1,2}|m{1,2}|s{1,2}|S{1,4}|X{1,5}|x{1,5}|O{1,4}|z{1,4}|t{1,2}|T{1,2}|P{1,4}|p{1,4}|yo|qo|Qo|Mo|lo|Lo|wo|do|io|eo|co|ho|Ho|Ko|ko|mo|so|Pp|PPpp|PPPppp|PPPPpppp|.)*',
    ...
    if (value.length === 0 || config_editHistory) {
        curWindow
            .showInputBox({
                prompt:
                    '语法: [<起始值>][:<步长>][#<重复次数>][*<频率>][~<格式>]r[+]<随机>][::<表达式>][@<停止表达式>][$][!]',
            ...
            switch (rest) {
                ...
                case '!':
                    if (histories.length > 0) {
                        ...
                        curWindow.showErrorMessage('[历史记录] 没有上一条命令');
                        return;
                    }
                    break;
                ...
                case 'p':
                    if (histories.length > 0) {
                        ...
                        curWindow.showErrorMessage('[历史记录] 为空');
                    }
                    return;
                // clear history
                case 'c':
                    context.globalState.update('histories', []);
                    curWindow.showErrorMessage('[历史记录] 已清空!');
                    return;
                default:
